...
(async () => {
  ...
  const data = await fs.readFileSync(
    __dirname + '/../output/資産運用・FRAX.csv',
    'utf-8'
  );
  ...
  try {
    ...
    for (i = 0; i < records.length; i++) {//URLとnameにCSVファイルのデータを格納し、仮想ブラウザーを立ち上げて、URLに格納したサイトに飛ぶ。
      ...
      try {
        ...
        const result = await page.evaluate(() => {
          ...
          const exponameTemp = Array.from(document.querySelectorAll('[data-dtm-category-name="出展展示会"] div span'))//展示会名を全て取得して配列格納し、','で区切って結合。
    ...
    fs.writeFileSync(`${outputdir}/資産運用EXPO関西・FRAX_OSAKA.csv`, outputData, {
      encoding: 'utf8',
    });
  } catch (error) {
    console.log(error);
