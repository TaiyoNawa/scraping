/*今回のデベロッパーツール
A1 = Array.from(document.querySelectorAll(".directory-item.directory-item-feature-toggled.exhibitor-category"))
...
A2 = A1.map(i => {
    name = i.querySelector("h3").innerText.replace(/\.,/g, ". "); //co.,LTD対策（要変更）(スペース付きだとダメかもだからCSVファイル内で検索)
    url = i.querySelector("a").getAttribute("href")
    return {name, url}
})

downloadCSVFile(convertCsvData(A2, ["name", "url"]), "lifestyleweek-summer.csv")
*/
...
(async () => {
  ...
  const data = await fs.readFileSync(
    __dirname + '/../output/lifestyleweek-summer.csv',
    'utf-8'
  );
...
/*             if(j.querySelector('.facebook-logo-color')){
    ...
    fs.writeFileSync(`${outputdir}/LifeStyleWeek-Summer.csv`, outputData, {
      encoding: 'utf8',
    });
  } catch (error) {
    console.log(error);
