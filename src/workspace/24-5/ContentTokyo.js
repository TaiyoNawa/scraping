...
(async () => {
  ...
  const data = await fs.readFileSync(
    __dirname + '/../output/ContentTokyo.csv',
    'utf-8'
  );
...
/*             if(j.querySelector('.facebook-logo-color')){
          ...
          console.log(exponame, tenji, komabango, website, tel, email, address, country);
          return({
    ...
    fs.writeFileSync(`${outputdir}/ContentTokyo.csv`, outputData, {
      encoding: 'utf8',
    });
  } catch (error) {
    console.log(error);
