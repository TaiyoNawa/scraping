...
(async () => {
  ...
  const data = await fs.readFileSync(
    __dirname + '/../output/JapanSportsWeek.csv',
    'utf-8'
  );
    ...
    for (i = 0; i < records.length; i++) {
      ...
      try {
        ...
        const result = await page.evaluate(() => {
          //スクレイピング  
          const tenjikai = document.querySelector('[data-dtm-category-name="展示会"] span')?.innerText;
...
/*             if(j.querySelector('.facebook-logo-color')){
          ...
          const tenjikaimeiTemp = Array.from(document.querySelectorAll('[data-dtm-category-name="展示会名"] div span'))//展示会名を全て取得して配列格納し、','で区切って結合
          if(tenjikaimeiTemp!=null){
            tenjikaimei = tenjikaimeiTemp.map(i => {
          ...
          console.log(tenjikaimei, tenjikai, komabango, website, tel, email, address, country);
          return({
            tenjikaimei,tenjikai, komabango, website, tel, email, address, country, 
          });
        }); //eveluate文終了
    ...
    fs.writeFileSync(`${outputdir}/JapanSportsWeek.csv`, outputData, {
      encoding: 'utf8',
    });
  } catch (error) {
    console.log(error);
