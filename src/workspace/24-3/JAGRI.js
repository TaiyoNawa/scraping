...
(async () => {
  ...
  const data = await fs.readFileSync(
    __dirname + '/../output/JAGRI-url.csv',
    'utf-8'
  );
...
/*             if(j.querySelector('.facebook-logo-color')){
          ...
          console.log(komabango, tenji, website, email, tel, country, address);
          return({
            komabango, tenji, website, email, tel, country, address          
          });
        }); //eveluate文終了
    ...
    fs.writeFileSync(`${outputdir}/JAGRI.csv`, outputData, {
      encoding: 'utf8',
    });
  } catch (error) {
    console.log(error);
