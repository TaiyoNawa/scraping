...
const fileName = 'ウェルネスフードジャパン夏.csv';


//今回のURL：https://cafewfj2024.reg-visitor.com/exhibitor
(async () => {
  try {
    //CSVファイル読み込み
    const data = await fs.readFileSync(
      __dirname + '/../output/ウェルネスフード.csv',
      'utf-8'
    );
    ...
    for (i = 0; i < records.length; i++) {
      ...
      koma = document.querySelector(".exhibitors-detail-top__booth-number").innerText.replace("小間番号：", "");
      kyoudou = document.querySelector(".exhibitors-detail-top__zone").innerText;
      a = document.querySelector(".recommend-list-section").querySelectorAll("tr")
    ...
    const result = { rawName, clickTarget, address, tel, email, website, koma, kyoudou };
    console.log(result);
    results.push(result)
    }
    // console.log(results);
